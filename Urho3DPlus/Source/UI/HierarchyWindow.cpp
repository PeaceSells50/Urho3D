...
namespace Urho3D
{
	HierarchyWindow::HierarchyWindow(Context* context) : Window(context)
	{
		...
		SubscribeToEvent(hierarchyList_, E_ITEMCLICKED, URHO3D_HANDLER(HierarchyWindow, HandleHierarchyItemClick));
	...
	void HierarchyWindow::HandleHierarchyItemClick(StringHash eventType, VariantMap& eventData)
	{

		if (eventData["Button"].GetInt() != MOUSEB_RIGHT)
			return;

		UIElement* uiElement = static_cast<UIElement*>(eventData["Item"].GetPtr());
		int selectionIndex = eventData["Selection"].GetInt();

		Vector<UIElement*> actions;
		int type = uiElement->GetVars()[TYPE_VAR]->GetInt();
		...
		if (type == ITEM_COMPONENT || type == ITEM_NODE)
		{
			if (input_->GetKeyDown(KEY_LSHIFT))
				hierarchyList_->AddSelection(selectionIndex);
			else
			{
		...
		{
			unsigned int id = uiElement->GetVars()[COMPONENT_ID_VAR]->GetUInt();
			Component* targetComponent = editorData_->GetEditorScene()->GetComponent( id );
			if (!targetComponent)
				return;

			actions.Push(editor_->CreateContextMenuItem("Copy", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextCopy)));
			actions.Push(editor_->CreateContextMenuItem("Cut", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextCut)));
			actions.Push(editor_->CreateContextMenuItem("Delete", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextDelete)));
			actions.Push(editor_->CreateContextMenuItem("Paste", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextPaste)));
			actions.Push(editor_->CreateContextMenuItem("Enable/disable", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextEnableDisable)));

			/* actions.Push(CreateBrowserFileActionMenu("Edit", "HandleBrowserEditResource", file)); */
		}
		else if (type == ITEM_NODE)
		{
			actions.Push(editor_->CreateContextMenuItem("Create Replicated Node", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextCreateReplicatedNode)));
			actions.Push(editor_->CreateContextMenuItem("Create Local Node", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextCreateLocalNode)));
			actions.Push(editor_->CreateContextMenuItem("Duplicate", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextDuplicate)));
			actions.Push(editor_->CreateContextMenuItem("Copy", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextCopy)));
			actions.Push(editor_->CreateContextMenuItem("Cut", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextCut)));
			actions.Push(editor_->CreateContextMenuItem("Delete", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextDelete)));
			actions.Push(editor_->CreateContextMenuItem("Paste", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextPaste)));
			actions.Push(editor_->CreateContextMenuItem("Reset to default", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextResetToDefault)));
			actions.Push(editor_->CreateContextMenuItem("Reset position", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextResetPosition)));
			actions.Push(editor_->CreateContextMenuItem("Reset rotation", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextResetRotation)));
			actions.Push(editor_->CreateContextMenuItem("Reset scale", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextResetScale)));
			actions.Push(editor_->CreateContextMenuItem("Enable/disable", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextEnableDisable)));
			actions.Push(editor_->CreateContextMenuItem("Unparent", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextUnparent)));
		}
		else if (type == ITEM_UI_ELEMENT)
		{
			// close ui element
			actions.Push(editor_->CreateContextMenuItem("Close UI-Layout", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextUIElementCloseUILayout)));
			actions.Push(editor_->CreateContextMenuItem("Close all UI-layouts", URHO3D_HANDLER(HierarchyWindow, HandleHierarchyContextUIElementCloseAllUILayouts)));
		}

		if (actions.Size() > 0)
			editor_->ActivateContextMenu(actions);
	}

	void HierarchyWindow::HandleHierarchyContextCopy(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextCut(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextDelete(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextPaste(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextEnableDisable(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextCreateReplicatedNode(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextCreateLocalNode(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextDuplicate(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextResetToDefault(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextResetPosition(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextResetRotation(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextResetScale(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextUnparent(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextUIElementCloseUILayout(StringHash eventType, VariantMap& eventData)
	{

	}

	void HierarchyWindow::HandleHierarchyContextUIElementCloseAllUILayouts(StringHash eventType, VariantMap& eventData)
	{
